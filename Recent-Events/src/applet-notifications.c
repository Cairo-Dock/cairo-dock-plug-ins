...
CD_APPLET_ON_CLICK_BEGIN
	cd_find_recent_events (0, (CDOnGetEventsFunc)_on_get_recent_events, myApplet);
...
static void _on_find_related_events (ZeitgeistResultSet *pEvents, Icon *pIcon)
{
	...
	while (zeitgeist_result_set_has_next (pEvents))
	{
		...
		for (i = 0; i < zeitgeist_event_num_subjects (event); i++)
		{
			...
			cCommand = g_strdup_printf ("%s \"%s\"", pIcon->cCommand, cEventURI);
...
CD_APPLET_ON_BUILD_MENU_PROTO
{
	...
	if (CD_APPLET_CLICKED_ICON != NULL && CD_APPLET_CLICKED_ICON->pMimeTypes != NULL)
	{
		...
		{
			cd_find_recent_related_files ((const gchar **)CD_APPLET_CLICKED_ICON->pMimeTypes, _on_find_related_events, CD_APPLET_CLICKED_ICON);
			s_pMenu = pAppletMenu;
			// ne marche po ...
			g_signal_connect (G_OBJECT (pAppletMenu), "destroy-event", G_CALLBACK (_on_delete_menu), NULL);
			g_signal_connect (G_OBJECT (pAppletMenu), "delete-event", G_CALLBACK (_on_delete_menu), NULL);
		}
	}
