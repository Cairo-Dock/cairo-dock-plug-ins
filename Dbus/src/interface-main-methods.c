...
{
	if (!strcmp (cMethod, "Reboot")) { _main_reboot (pPar, pInv); return; }
	if (!strcmp (cMethod, "Quit")) { _main_quit (pPar, pInv); return; }
	if (!strcmp (cMethod, "ShowDock")) { _main_show_dock (pPar, pInv); return; }
	if (!strcmp (cMethod, "ShowDesklet")) { _main_show_desklet (pPar, pInv); return; }
	if (!strcmp (cMethod, "SetQuickInfo")) { _main_set_quick_info (pPar, pInv); return; }
	if (!strcmp (cMethod, "SetLabel")) { _main_set_label (pPar, pInv); return; }
	if (!strcmp (cMethod, "SetIcon")) { _main_set_icon (pPar, pInv); return; }
	if (!strcmp (cMethod, "SetEmblem")) { _main_set_emblem (pPar, pInv); return; }
	if (!strcmp (cMethod, "Animate")) { _main_animate (pPar, pInv); return; }
	if (!strcmp (cMethod, "DemandsAttention")) { _main_demands_attention (pPar, pInv); return; }
	if (!strcmp (cMethod, "ShowDialog")) { _main_show_dialog (pPar, pInv); return; }
	if (!strcmp (cMethod, "SetMenu")) { _main_set_menu (pPar, pInv); return; }
	if (!strcmp (cMethod, "SetProgress")) { _main_set_progress (pPar, pInv); return; }
	if (!strcmp (cMethod, "Add")) { _main_add (pPar, pInv); return; }
	if (!strcmp (cMethod, "Reload")) { _main_reload (pPar, pInv); return; }
	if (!strcmp (cMethod, "Remove")) { _main_remove (pPar, pInv); return; }
	if (!strcmp (cMethod, "GetProperties")) { _main_get_properties (pPar, pInv); return; }
	
	g_dbus_method_invocation_return_error (pInv, G_DBUS_ERROR, G_DBUS_ERROR_UNKNOWN_METHOD, "Unknown method: '%s'", cMethod);
}
