...
void rendering_load_controler_data (CairoDesklet *pDesklet, cairo_t *pSourceContext)
{
	...
	if (pControler->b3D)
	{
		pControler->iEllipseHeight = MIN (pDesklet->pIcon->fHeight, pDesklet->iHeight - 2 * (g_iconTextDescription.iSize + g_fReflectSize) - 1);
...
void rendering_load_icons_for_controler (CairoDesklet *pDesklet, cairo_t *pSourceContext)
{
	...
	if (pControler->b3D)
	{
		fCentralSphereWidth = MAX (1, (MIN (pDesklet->iWidth, pDesklet->iHeight - g_iconTextDescription.iSize) - g_iDockRadius) * CONTROLER_RATIO_ICON_DESKLET - g_fReflectSize);
	...
	else
	{
		...
		fCentralSphereHeight = MAX (1, (pDesklet->iHeight - g_iDockRadius - g_iconTextDescription.iSize) * CONTROLER_RATIO_ICON_DESKLET);
	}
	...
	if (pIcon != NULL)
	{
		...
		pIcon->fDrawY = g_iconTextDescription.iSize + g_iDockRadius/2;
		pIcon->fScale = 1.;
		pIcon->fAlpha = 1.;
	...
	double fX = g_iDockRadius + pControler->fGapBetweenIcons, fY = g_iconTextDescription.iSize + pDesklet->pIcon->fHeight + g_fReflectSize;
	GList* ic;
	for (ic = pDesklet->icons; ic != NULL; ic = ic->next)
	{
		pIcon = ic->data;
		...
		else
		{
			...
			pIcon->fHeight = MAX (1, (pDesklet->iHeight - g_iDockRadius - g_iconTextDescription.iSize) *(1 - CONTROLER_RATIO_ICON_DESKLET));
		}
...
void rendering_draw_controler_in_desklet (cairo_t *pCairoContext, CairoDesklet *pDesklet, gboolean bRenderOptimized)
{
	...
	if (pControler->b3D)
	{
		...
		cairo_save (pCairoContext);
		pDesklet->pIcon->fDrawY = g_iconTextDescription.iSize;
	...
	else
	{
		...
		double fX = g_iDockRadius + pControler->fGapBetweenIcons, fY = g_iconTextDescription.iSize + pDesklet->pIcon->fHeight;
		for (ic = pDesklet->icons; ic != NULL; ic = ic->next)
		{
			pIcon = ic->data;
