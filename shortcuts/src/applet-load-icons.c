...
void _manage_event_on_drive (CairoDockFMEventType iEventType, const gchar *cBaseURI, GList *pIconsList, CairoContainer *pContainer, CairoDockModuleInstance *myApplet)
{
	...
	switch (iEventType)
	{
		...
		case CAIRO_DOCK_FILE_CREATED :  // un point de montage a ete connecte.
		{
			...
			cairo_dock_show_temporary_dialog_with_icon_printf (
				bIsMounted ? _("%s is now mounted") : _("%s has been connected"),
				pNewIcon, pContainer,
				4000,
		...
		case CAIRO_DOCK_FILE_MODIFIED :  // un point de montage a ete (de)monte
		{
			...
			g_free (cUri);
			cairo_dock_show_temporary_dialog_with_icon_printf (
				bIsMounted ? _("%s is now mounted") : _("%s is now unmounted"),
