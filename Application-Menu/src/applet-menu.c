                      ...
                      DbusmenuClient * client)
{
  ...
  myApplet.transport_widgets_list = g_list_append ( myApplet.transport_widgets_list, bar );
                         ...
                         DbusmenuClient * client)
{
  ...
  if (myApplet.volume_widget != NULL){ 
    volume_widget_tidy_up (myApplet.volume_widget);
    gtk_widget_destroy (myApplet.volume_widget);
    myApplet.volume_widget = NULL;
  }
  volume_widget = volume_widget_new (newitem);
  myApplet.volume_widget = volume_widget;

  GtkWidget* ido_slider_widget = volume_widget_get_ido_slider(VOLUME_WIDGET(myApplet.volume_widget));
                        ...
                        DbusmenuClient * client)
{
  ...
  if (myApplet.voip_widget != NULL){ 
    voip_input_widget_tidy_up (myApplet.voip_widget);
    gtk_widget_destroy (myApplet.voip_widget);
    myApplet.voip_widget = NULL;
  }

  voip_widget = voip_input_widget_new (newitem);
  myApplet.voip_widget = voip_widget;
...
key_press_cb(GtkWidget* widget, GdkEventKey* event, CairoDockModuleInstance *myApplet)
{
  ...
  if (IDO_IS_SCALE_MENU_ITEM(menuitem) == TRUE){
    ...
    if (g_ascii_strcasecmp (ido_scale_menu_item_get_primary_label (IDO_SCALE_MENU_ITEM(menuitem)), "VOLUME") == 0) {
      ...
      GtkWidget* slider_widget = volume_widget_get_ido_slider(VOLUME_WIDGET(myApplet.volume_widget));
      ...
      GtkWidget* slider_widget = voip_input_widget_get_ido_slider(VOIP_INPUT_WIDGET(myApplet.voip_widget));
      GtkWidget* slider = ido_scale_menu_item_get_scale((IdoScaleMenuItem*)slider_widget);
      GtkRange* range = (GtkRange*)slider;
      g_return_val_if_fail(GTK_IS_RANGE(range), FALSE);
      current_value = gtk_range_get_value(range);
    ...
    if (new_value != current_value){
      if (is_voip_slider == TRUE){
        voip_input_widget_update (VOIP_INPUT_WIDGET(myApplet.voip_widget), new_value);
      }
      else{
        volume_widget_update (VOLUME_WIDGET(myApplet.volume_widget), new_value, "keypress-update");
      }
    }
  }
    ...
    for ( elem = myApplet.transport_widgets_list; elem; elem = elem->next ) {
