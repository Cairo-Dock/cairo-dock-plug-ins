...
void cd_opengl_load_external_conf_theme_values (CairoDockModuleInstance *myApplet)
{
	...
	cd_debug ("MP_YDU : %s", myConfig.cThemePath);
...
void cd_opengl_scene (CairoDockModuleInstance *myApplet, int iWidth, int iHeight)
{
	...
	if (myConfig.iExtendedMode == MY_DESKLET_CAROUSSEL || myConfig.iExtendedMode == MY_DESKLET_CONTROLER)
		glTranslatef (iWidth, iHeight, 0);
	else
